...
def create_new_day(year, day):
    ...
    if day == 0:
        ...
        if day >= MAX_DAY:
            return (
                False,
                (
                    f"Cannot create new day because the days for {year} "
                    f"already reached maximum {MAX_DAY}"
                ),
            )
    ...
    if not os.path.isdir(day_dir):
        ...
        return (
            False,
            (
                f"{day_dir} is already created before! "
    ...
    return (
        True,
        f"{tc.BLUE} {day_dir}{tc.ENDC} folder has been created and ready to use.",
    )
...
def run(year, day, parser):
    ...
    print(
        (
            ...
            f"day => {tc.GREEN + tc.ULINE + str(day).zfill(2) + tc.ENDC + tc.HEADER} }}"
            f"{tc.ENDC}"
        )
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Advent Of Code Solutions by Gökhan Öztürk"
    )
    parser.add_argument(
        "year", metavar="year", type=int, default=CURRENT_YEAR, help="Event year"
    )
    ...
    parser.add_argument(
        "-r", "--run", action="store_true", help="Runs the given day in the given year."
    )
    parser.add_argument(
        "-i",
        "--info",
        action="store_true",
        help="Prints the challenge info of given year and day.",
    )
    ...
    if not args.create and args.year < START_YEAR or args.year > CURRENT_YEAR:
        parser.error(
            f"Year cannot be lower than {START_YEAR} or greater than {CURRENT_YEAR}"
        )
        return
